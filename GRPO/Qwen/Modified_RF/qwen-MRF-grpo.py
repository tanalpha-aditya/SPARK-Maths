...
WANDB_PROJECT = "QwenMATH-GRPO"  # Changed project name
...
def standardize_number(text: str) -> str:
    text = text.replace(",", "")
    try:
        if "/" in text:
            parts = text.split("/")
...
def extract_boxed_answer(text: str) -> str:
    ...
    match = re.search(r"\\boxed\{([\d\.,/]+)\}", text)  # Allow commas, periods, slashes
    if match:
        extracted = match.group(1)
        return standardize_number(extracted)
...
def extract_hash_answer(text: str) -> str | None:
    ...
    return standardize_number(answer_text)
...
def get_gsm8k_questions(split="train") -> Dataset:
    ...
    data = data.map(
        lambda x: {
            ...
            "answer": extract_hash_answer(x["answer"]),  # Standardized answer
        }
    )
    return data
...
def correctness_reward_func(prompts: List[List[Dict[str, str]]], completions: List[List[Dict[str, str]]], answer: List[str], **kwargs) -> List[float]:
    ...
    for i in range(len(prompts)):
        ...
        for extracted, corr_ans in zip(extracted_responses, correct_answers):
            if extracted == "N/A" or corr_ans == "N/A":
                rewards.append(0.0)
...
def main(test_dataset_path=None):
    ...
    model_output_path = os.path.join(OUTPUT_DIR, "QWEN-Math-GRPO-NoXML")  # Changed output path
